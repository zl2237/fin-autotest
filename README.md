...
本项目面向物流管理系统，采用分层设计，支持从**新建订单**到**应付对账**的 19 条链路端到端测试，覆盖订单全生命周期、审批流、费用单生成、发票上传与登记、应收核销、应付对账。

**核心能力：**
- 19 条链路，覆盖从新建到应付对账完整流程（按需执行，灵活组合）
...
│
├── workflows/                   # 流程编排层
│   ├── order_workflow.py        # 订单全流程编排（新建→分发→暂存→提交→子订单→录费用→审批→费用单→对账→开票→发票上传→应收核销→应付对账）
...
| link19 | 应付对账 | ... → **应收核销** → **应付对账**（financePayList + orderPayAccountEdit） |

> 链路按依赖顺序递增：link19 隐含 link1~link18 的全部步骤。
...
OrderWorkflow.run_until_payable_account()    # 到应付对账（link19）
```
