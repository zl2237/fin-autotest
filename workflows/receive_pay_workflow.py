"""
订单流程编排 - receive_pay 工作流（可选：订单 → 应收 → 应付）

本文件按链路依赖树语义，将应收阶段（13~18）前置到应付阶段（21~25）之前执行。
与 pay_receive_workflow.py 共同覆盖两类 workflow 类型。
"""
import logging
...
class ReceivePayWorkflow:
    """
    可选 workflow：订单 → 应收 → 应付

    流程阶段：新建 → 分发 → [暂存] → 提交 → 生成子订单 → 录费用 → 审批流程
              → 应收对账 → 应收开票 → 应收核销 → 应付对账 → 应付开票 → 付款需求
    ...
    def run(cls, bl_no: str = None, stop_at: str = 'submit', skip_stash: bool = False, fee_configs: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        执行完整流程：订单 → 应收 → 应付，stop_at 控制停止阶段
        ...
        Args:
            ...
            stop_at: 执行到哪个阶段后停止，可选值：
                ...
                - 'receive_account'    新建 + ... + 生成费用确认单 + 发起应收对账批次
        ...
        Returns:
            与 OrderWorkflow.run 相同的 result 结构，并追加应收/应付阶段结果
        """
        ...
        # receive_pay: 应收 → 应付
        # 如果 stop_at 在应付阶段，需要先把应收跑完（stop_at='receive_writeoff'），再跑应付
        if stop_at in pay_stops:
            order_result = ReceiveWorkflow.run(bl_no=bl_no, stop_at='receive_writeoff', fee_configs=fee_configs, order_result=order_result)
