"""
测试链路 30：订单+应收（receive_pay），应付发票上传与登记
执行顺序：订单 1~12 步 + 应收 1~6 步 + 应付 1~4 步
前置条件：应付 2 步完成
"""
...
@pytest.mark.order_receive_pay10
class TestOrderReceivePay10PayableInvoiceRegister:

    @allure.severity("critical")
    def test_order_receive_pay10_payable_invoice_register(self):
        bl_no = generate_bl_no(210)

        with allure.step("执行完整订单流程，运行至应付发票上传与登记节点 payable_invoice_register"):
