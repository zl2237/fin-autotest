"""
测试链路 21：订单+应收（receive_pay），发起应收对账批次
...
class TestOrderReceivePay1ReceiveAccount:

    @allure.severity("critical")
    def test_order_receive_pay1_receive_account(self):
        bl_no = generate_bl_no(201)

        with allure.step("执行完整订单流程，运行至发起应收对账批次节点 receive_account"):
