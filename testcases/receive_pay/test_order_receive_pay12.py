"""
测试链路 32：订单+应收（receive_pay），审核生成付款单
执行顺序：订单 1~12 步 + 应收 1~6 步 + 应付 1~6 步
前置条件：应付 3 步完成
"""
...
@pytest.mark.order_receive_pay12
class TestOrderReceivePay12PayDemandAudit:

    @allure.severity("critical")
    def test_order_receive_pay12_pay_demand_audit(self):
        bl_no = generate_bl_no(212)

        with allure.step("执行完整订单流程，运行至审核生成付款单节点 pay_demand_audit"):
