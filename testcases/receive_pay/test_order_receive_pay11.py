"""
测试链路 31：订单+应收（receive_pay），发起付款需求
执行顺序：订单 1~12 步 + 应收 1~6 步 + 应付 1~5 步
前置条件：应付 3 步完成
"""
...
@pytest.mark.order_receive_pay11
class TestOrderReceivePay11PayDemand:

    @allure.severity("critical")
    def test_order_receive_pay11_pay_demand(self):
        bl_no = generate_bl_no(211)

        with allure.step("执行完整订单流程，运行至发起付款需求节点 pay_demand"):
