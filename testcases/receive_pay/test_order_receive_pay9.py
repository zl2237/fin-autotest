"""
测试链路 29：订单+应收（receive_pay），发起应付开票批次申请
执行顺序：订单 1~12 步 + 应收 1~6 步 + 应付 1~3 步
前置条件：应付 2 步完成
"""
...
@pytest.mark.order_receive_pay9
class TestOrderReceivePay9PayableInvoiceApply:

    @allure.severity("critical")
    def test_order_receive_pay9_payable_invoice_apply(self):
        bl_no = generate_bl_no(209)

        with allure.step("执行完整订单流程，运行至发起应付开票批次申请节点 payable_invoice_apply"):
