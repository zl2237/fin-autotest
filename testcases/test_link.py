"""
链路测试 - 验证新建→分发→暂存→提交→生成子订单→录费用→录审批→录订单锁定审批 完整链路数据流
...
标记说明：
  ...
  link7  - 新建、分发、查询、暂存、提交、生成子订单、录费用、录审批
  link8  - 新建、分发、查询、暂存、提交、生成子订单、录费用、录审批、录订单锁定审批
"""
import allure
import pytest
...
# =============================================================================
# 链路7：新建、分发、查询、暂存、提交、生成子订单、录费用、录审批
# =============================================================================
@pytest.mark.link7
class TestLink7RecordAudit:
    ...
    @allure.feature("链路测试")
    @allure.story("链路7：新建、分发、查询、暂存、提交、生成子订单、录费用、录审批")
    @allure.severity("critical")
    @allure.title("链路7：新建 → 分发 → 查询 → 暂存 → 提交 → 生成子订单 → 录费用 → 录审批")
    def test_link7_record_audit(self):
        """验证：完整链路（包含录审批），链路停在 record_audit 阶段"""
        ...
        with allure.step('执行链路（新建→分发→查询→暂存→提交→生成子订单→录费用→录审批）'):
            result = OrderWorkflow.full_flow(
