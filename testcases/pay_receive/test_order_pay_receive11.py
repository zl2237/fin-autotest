"""
测试链路 23：订单+应付（pay_receive），完整流程
...
@pytest.mark.order_pay_receive11
class TestOrderPayReceive11InvoiceBatchAudit:

    @allure.severity("critical")
    def test_order_pay_receive11_invoice_batch_audit(self):
        bl_no = generate_bl_no(111)

        with allure.step("执行完整订单流程，运行至应收开票批次审核节点 invoice_batch_audit"):
        ...
        with allure.step("校验：应收开票批次及批次审核接口返回正常"):
            ...
            assert invoice_batch_result["page_data"].get("code") == 200
            assert invoice_batch_result.get("batch_id")
