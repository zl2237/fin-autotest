"""
测试链路 17：订单+应付（pay_receive），完整流程
...
from testcases.pay_receive.helpers import _build_fee_config, _assert_link12_prerequisite_ok, _assert_payable_account_ok, _assert_confirm_payable_ok, _assert_payable_invoice_apply_ok, _assert_payable_invoice_register_ok, _assert_pay_demand_ok


@pytest.mark.order_pay_receive5
class TestOrderPayReceive5PayDemand:

    @allure.severity("critical")
    def test_order_pay_receive5_pay_demand(self):
        bl_no = generate_bl_no(105)

        with allure.step("执行完整订单流程，运行至付款申请节点 pay_demand"):
