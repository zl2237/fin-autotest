"""
链路测试 - pay_receive 流程公共辅助函数

适用于 order_pay_receive1 ~ order_pay_receive13 的共享断言逻辑。
"""
...
def _assert_link12_prerequisite_ok(result):
    """
    order_pay_receive1 的前置链路（order1~12）断言：
...
def _assert_audit_and_fee_doc_ok(result):
    """order_pay_receive8~13 共用的"录费用+审批+费用单据"链式断言。"""
    assert result['create_data']['code'] == 200, f'新建失败: {result["create_data"]}'
    assert result['distribute_data']['code'] == 200, f'分发失败: {result["distribute_data"]}'
    gen_data = result['generate_sub_data']
    assert gen_data['code'] == 200, f'生成子订单失败: {gen_data}'
...
def _assert_receive_account_ok(result):
    """order_pay_receive8 共用"发起应收对账批次"断言。"""
...
def _assert_confirm_receive_ok(result):
    """
    order_pay_receive10 确认应收对账断言：receiveAccountDetail + receiveConfirmList + accountConfirm 全部成功。
    """
...
def _assert_payable_account_ok(result):
    """
    order_pay_receive1 应付对账断言：financePayList + orderPayAccountEdit 全部成功。
    """
    payable_result = result.get('payable_account_result')
    assert payable_result is not None, (
        f'payable_account_result 不应为空（result keys: {list(result.keys())}）'
    )

    pay_list_resp = payable_result['pay_list_resp']
    pay_list_data = payable_result['pay_list_data']
    assert isinstance(pay_list_data, dict), (
        f'pay_list_data 应为 dict，实际为 {type(pay_list_data)}: {str(pay_list_data)[:500]}'
    )
...
def _assert_confirm_payable_ok(result):
    """
    order_pay_receive2 确认应付对账断言：payAccountPage + accountConfirm 全部成功。
    """
...
def _assert_payable_invoice_apply_ok(result):
    """
    order_pay_receive3 发起应付开票批次申请断言：financePayList（开票）+ getOrderInfoByFeeId + batchOrderEdit 全部成功。
    """
...
def _assert_payable_invoice_register_ok(result):
    """
    order_pay_receive4 应付发票上传与登记断言。
    """
