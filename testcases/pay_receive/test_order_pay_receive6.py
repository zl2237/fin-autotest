"""
测试链路 18：订单+应付（pay_receive），完整流程
...
from testcases.pay_receive.helpers import _build_fee_config, _assert_link12_prerequisite_ok, _assert_payable_account_ok, _assert_confirm_payable_ok, _assert_payable_invoice_apply_ok, _assert_payable_invoice_register_ok, _assert_pay_demand_ok, _assert_pay_demand_audit_ok


@pytest.mark.order_pay_receive6
class TestOrderPayReceive6PayDemandAudit:

    @allure.severity("critical")
    def test_order_pay_receive6_pay_demand_audit(self):
        bl_no = generate_bl_no(106)

        with allure.step("执行完整订单流程，运行至付款申请审核节点 pay_demand_audit"):
